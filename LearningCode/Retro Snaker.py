...
class Snake(object):
    ...
    def move(self):#已知direction 和 food 位置，move一下并画出来，不断调用move函数进行覆盖刷新
        print(self.body)
        head = self.body[0]
        ...
        if not new in self.available_grid:#此次移动完成后，判断游戏是否可以继续，若不可以
            self.status.reverse()#修改游戏继续参数的值
            self.game_over = True
        else:
            self.grid.draw(new,color=self.color)#若可以继续，画出头部元素

class SnakeGame(Frame):#获得键盘输入，并不断刷新界面，调用move函数，要用bind_all函数，必须继承！！！！！！继承！！！
    def __init__(self,root=None,*args,**kwargs):
        Frame.__init__(self,root)
        self.grid=Grid(root=root,*args,**kwargs)
        self.snake=Snake(self.grid)
        self.bind_all('<KeyRelease>',self.key_release)#估计 也是 继承 下来的
        #snakegame类是从父类继承下来的，所以也具有 bind_all 方法
